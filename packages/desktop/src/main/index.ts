...
const APP_IDS: Record<string, string> = {
  dev: "ai.am.desktop.dev",
  beta: "ai.am.desktop.beta",
  prod: "ai.am.desktop",
}
const TEST_ONBOARDING = process.env.AM_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.AM_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.am.desktop.dev"
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("am://"))
    if (urls.length) {
  ...
  const needsMigration = ((): boolean => {
    if (process.env.AM_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "am", "am.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.AM_PORT
    if (fromEnv) {

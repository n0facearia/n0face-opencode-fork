...
const originalOpencodePort = process.env.AM_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.AM_EDITOR_SSE_PORT = originalOpencodePort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.AM_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with AM_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.AM_EDITOR_SSE_PORT = "4020"

...
`,
  ...
  const pure = process.env.AM_PURE
  const meta = process.env.AM_PLUGIN_META_FILE
  process.env.AM_PURE = "1"
  process.env.AM_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.AM_PURE
    } else {
      process.env.AM_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.AM_PLUGIN_META_FILE
    } else {
      process.env.AM_PLUGIN_META_FILE = meta
    }
  }
})

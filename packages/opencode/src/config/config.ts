...
export const Info = Schema.Struct({
  ...
  autoupdate: Schema.withConstructorDefault(Effect.succeed(false))(Schema.optional(Schema.Union([Schema.Boolean, Schema.Literal("notify")]))).annotate({
    description:
...
function globalConfigFile() {
  const candidates = configFileNames().map((file) => path.join(Global.Path.config, file))

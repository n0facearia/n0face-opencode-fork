...
export class Service extends Context.Service<Service, Interface>()("@am/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.AM_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.AM_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.AM_DISABLE_MODELS_FETCH) return {}
    ...
    if (!Flag.AM_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {

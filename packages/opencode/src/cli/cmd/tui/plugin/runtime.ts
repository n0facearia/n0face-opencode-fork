...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const configDir = process.env.AM === "1" ? ".am" : ".opencode"
    const local_dir =
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, process.env.AM === "1" ? ".am" : ".opencode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void }) {
  ...
  runtime = next
  try {
    const records = Flag.AM_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.AM_PURE && config.plugin_origins?.length) {

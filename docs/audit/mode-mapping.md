...
|---|---|---|---|---|
| 1 | **Agent definitions** | `.opencode/agent/triage.md`, `.opencode/agent/duplicate-pr.md` | `.n0face/agent/` | **Refactor** — Move agent configs to `.n0face/agent/` structure; n0face modes (cleanup, design, security) already exist there |
| 2 | **Command definitions** | `.opencode/command/*.md` (8 files) | `.n0face/command/` | **Refactor** — Move commands; `.n0face/command/` already has `import-md.md` and `new-project.md` |
| 3 | **Mode system prompts** | `src/session/prompt/*.txt` (12 provider-specific prompts) | `.n0face/*.mode.md` | **Refactor** — Replace provider-specific prompts with mode-specific prompts (cleanup, design, security) |
...
| 5 | **Config system** | `packages/opencode/src/config/` (23 files) | `.n0face/` + `packages/n0face/src/config/` | **Keep** — Config system is already Effect-based and layered; needs rebranding |
...
| 10 | **Memory/persistence** | `packages/opencode/src/storage/` (SQLite + Drizzle) | `.n0face/state/` | **Refactor** — Extract state schema to `.n0face/state/`; keep storage engine |
...
| 16 | **Skill system** | `packages/opencode/src/skill/` + `.opencode/skills/` | `.n0face/skills/` | **Refactor** — Move skills under `.n0face/` |
...
| 32 | **Glossary/i18n** | `.opencode/glossary/` (16 languages) | `.n0face/glossary/` | **Refactor** — Move translation glossaries to `.n0face/` |
| 33 | **Themes** | `.opencode/themes/` + `packages/ui/src/theme/` | `.n0face/themes/` | **Refactor** — Move custom themes to `.n0face/` |
| 34 | **UI config** | `.opencode/tui.json` | `.n0face/tui.json` | **Refactor** — Move TUI config to `.n0face/` |
| 35 | **Main config** | `.opencode/opencode.jsonc` | `.n0face/n0face.jsonc` | **Refactor** — Rename to n0face config |
| 36 | **Env declarations** | `.opencode/env.d.ts` | `.n0face/env.d.ts` | **Refactor** — Move type declarations |
| 37 | **Custom tools** | `.opencode/tool/github-triage.ts`, `github-pr-search.ts` | `.n0face/tool/` | **Refactor** — Move custom tools to `.n0face/` |
| 38 | **Plugins** | `.opencode/plugins/` | `.n0face/plugins/` | **Refactor** — Move plugins to `.n0face/` |
| 39 | **Package dependencies** | `.opencode/package.json` | `.n0face/package.json` | **Keep** — Already mostly duplicated; `.n0face/package.json` already exists |
...
| 54 | **n0face plan.md** | Root `n0face project plan.md` (empty) | `.n0face/plan.md` | **Create** — Migrate plan to `.n0face/` |
...
|--------|-----------------|------------------|-------------------|
| **Agent/mode definitions** | `.opencode/agent/*.md` | `.n0face/agent/*.md` | Move files and adapt to mode-based routing |
| **Command definitions** | `.opencode/command/*.md` | `.n0face/command/*.md` | Move files, add n0face-specific commands |
| **Mode system prompts** | `src/session/prompt/*.txt` | `.n0face/*.mode.md` | Migrate from provider-specific prompts to mode-specific prompts (cleanup, design, security) |
| **Main config** | `.opencode/opencode.jsonc` | `.n0face/n0face.jsonc` | Rename; update all config path references |
| **TUI config** | `.opencode/tui.json` | `.n0face/tui.json` | Move |
| **Skills** | `.opencode/skills/` | `.n0face/skills/` | Move and re-index |
| **Themes** | `.opencode/themes/` | `.n0face/themes/` | Move |
| **Custom tools** | `.opencode/tool/` | `.n0face/tool/` | Move |
| **Plugins** | `.opencode/plugins/` | `.n0face/plugins/` | Move |
| **Glossary/i18n** | `.opencode/glossary/` | `.n0face/glossary/` | Move |
| **State schema** | `src/storage/schema.sql.ts` | `.n0face/state/` | Extract schema definitions to `.n0face/state/` for portability |
| **Memory/prompt system** | `src/session/prompt.ts` (2135 lines) | `.n0face/agent/` + refactored prompt loader | Decouple static prompts from agent; move mode prompts out of code |
| **Config paths** | `src/config/paths.ts` | `.n0face/` paths | Update all path constants from `.opencode/` to `.n0face/` |
...
| **Upstream .opencode content** | `.opencode/` (entire directory) | Replace with `.n0face/` |
...
| **n0face project plan.md** | Root placeholder file (empty) | Replace with proper plan in `.n0face/plan.md` |

## 5. Hybrid Systems (Keep + Extend)

| System | Keep | Extend/Add |
|--------|------|------------|
| **Agent runtime** (`src/agent/agent.ts`) | Agent lifecycle, subagent spawning, tool execution | Mode-aware agent selection (read mode from `.n0face/`) |
| **Prompt loading** (`src/session/prompt.ts`) | Prompt assembly, model-specific templating | Load mode prompts from `.n0face/*.mode.md` instead of hardcoded `*.txt` files |
...
| **Mode context loader** | Load `.n0face/*.mode.md` as system prompt | High |
| **n0face.jsonc** | Main config file replacing `opencode.jsonc` | High |
| **State directory** | `.n0face/state/` for session/memory persistence | Medium |
...
| **n0face docs** | New documentation in `.n0face/` replacing opencode docs | Low |
...
.opencode/                  →        .n0face/ (already exists, merge contents)
```
